...
function bigquery
    ...
    if [ $sub_command = "list" ]
        ...
        if set -q _flag_columns # -c --columns
            set command "bq show --schema $project_id $location $_flag_dataset.$_flag_table"
            if set -q _flag_format # -f --format
                set val "$(bq show --schema $format $project_id $location $_flag_dataset.$_flag_table)"
            else #   no format
                ...
                if test -e $filename ; and not set -q _flag_overwrite
                    ...
                    set val "$(bq show --schema $project_id $location $_flag_dataset.$_flag_table | jq -r '.[] | .name + ": [" + .type + ", " + .mode + "] - (" + .description + ")"')"
                    mkdir -p $dir
                    echo $val > "$filename"
                end
            ...
            if test -e $filename ; and not set -q _flag_overwrite
                ...
                set val "$(bq ls --format=prettyjson $project_id $location $_flag_dataset | jq -r '.[].tableReference.tableId' ) "
                mkdir -p $dir
                echo $val > "$filename"
            end
        else
            ...
            set val "$(bq ls $format $project_id $location $_flag_dataset)"
        end
        ...
        set query_str (cat $query_path | string collect)

        # -------------------------------------------------------------------------------------------------
        # REPLACE VALUES IN QUERY
        if set -q _flag_dataset
            set query_str  ( echo "$query_str" | sed -e "s/sbRDOuWobz16XG953Vms/$_flag_dataset/g" | string collect)
            set query_str  ( echo "$query_str" | sed -e "s/DATASET_ID_VALUE/$_flag_dataset/g" | string collect)
        end

        if set -q _flag_table
            set query_str  ( echo "$query_str" | sed -e "s/TABLE_ID_VALUE/$_flag_table/g" | string collect)
        end
        ...
        if set -q _flag_trip_ids
            ...
            for id in (string split \n -- $trips)
                ...
                set val_temp  ( echo "$query_str" | sed -e "s/TRIP_ID_VALUE/$id/g" | string collect)
                # echo "$query_str" > $filename
                echo "$(echo "$val_temp" | bq query --use_legacy_sql=false $project_id $location --max_rows=$_flag_max_rows --allow_large_results=true --format=csv)" >  $filename
        ...
        if string match -q "*TABLE_ID_VALUE*" "$query_str"
            echo "The query requires a table to be specified"
        end
        ...
        if set -q _flag_filename
            echo "$(echo "$query_str" | bq query --use_legacy_sql=false $project_id $location --max_rows=$_flag_max_rows --allow_large_results=true --format=csv)" > "$_flag_filename" 
            echo "Number of rows in output file: $(sed -n '$=' $_flag_filename)"
            echo "The maximum number of rows is: $_flag_max_rows"
        else
            echo "$(echo "$query_str" | bq query --use_legacy_sql=false $format $project_id $location --max_rows=8000 --allow_large_results=true)"
        end
