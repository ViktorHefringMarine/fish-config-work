function __tmuxinator_using_command
...
function __tmuxinator_with_no_args
  set cmd (commandline -opc)
  if [ (count $cmd) -gt 1 ]
    return 1 
  end
...
complete -f -c $__fish_tmuxinator_program_cmd   -n '__tmuxinator_with_no_args' -a 'start open copy delete'
complete -f -c $__fish_tmuxinator_program_cmd   -n '__tmuxinator_using_command' -a 'nonono'
